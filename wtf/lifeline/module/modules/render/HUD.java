...
import wtf.lifeline.events.rendering.EventRender2D;
...
import wtf.lifeline.utils.render.*;
...
public class HUD extends Module {
	...
	public Mode<Enum> trmode = new Mode<Enum>("TitleMode", "TitleMode", TRMode.values(), TRMode.Topline);
	...
	public HUD() {
		super("HUD", new String[]{"gui"}, Category.RENDER);
		this.setColor(new Color(random.nextInt(255), random.nextInt(255), random.nextInt(255)).getRGB());
		this.setEnabled(true);
		this.setRemoved(true);
		this.addValues(this.mode, this.colormode, this.widgetmode, this.r, this.g, this.b, this.alpha,this.trmode,
				this.compassvalue, this.notification, this.crosshair, this.wingvalue, this.fpshurtcam,blackhotbar,
				this.onscreenradar, this.movescoreboardx, this.movescoreboardy, this.x, this.y, this.hidedebugline, this.rendertabui);
	}
	...
	private void renderHud(EventRender2D event) {
		if(this.notification.getValue()){
			NoticeNotification.drawNotifications();
		}
		...
		if (!this.mc.gameSettings.showDebugInfo) {
			...
			if (this.crosshair.getValue()) {
				int i = RenderUtils.width();
				int j = RenderUtils.height();
...
//            	}else {
				RenderUtil.drawCustomImage(RenderUtils.width() / 2 + 300 - width,
						RenderUtils.height() - height - (mc.ingameGUI.getChatGUI().getChatOpen() ? 14 : 0), width,
...
//            else if (!(boolean)this.customlogo.getValue()) {
			...
			if (trmode.getValue().equals(TRMode.Topline)) {
				RenderUtil.drawRect(2, 2, 4 + font.getStringWidth("  Windy |  |" + new Date(System.currentTimeMillis()) + mc.thePlayer.getName()), 3, new Color(r.getValue().intValue(), g.getValue().intValue(), b.getValue().intValue(), 255).getRGB());
				RenderUtil.drawRect(2, 3, 4 + font.getStringWidth("  Windy |  |" + new Date(System.currentTimeMillis()) + mc.thePlayer.getName()), 14, new Color(30, 30, 30, 80).getRGB());
				font.drawString(" Windy | " + new Date(System.currentTimeMillis()) + " | " + mc.thePlayer.getName(), 2 , 6, new Color(255, 255, 255).getRGB());
			}
			else {
				font.drawString("Windy (" + new Date(System.currentTimeMillis()) + ")", 2, 2, new Color(255, 255, 255).getRGB());
			}
			...
			for (Module m : sorted) {
				...
				float x = RenderUtils.width() - m.getAnimx();
							...
							if (nextModule != null) {
								if (this.getNextEnabledModule(sorted, nextIndex).getSuffix() != "") {
									...
									Gui.drawRect(x - 7d, y + 11d,
											RenderUtils.width() - font.getStringWidth(
...
//	                    	Gui.drawRect(x - 9, y + 1, x - 7, y + 9, new Color(0,0,0,90).getRGB());
						...
						case "OutLine": {
							...
							if (nextModule != null) {
								if (this.getNextEnabledModule(sorted, nextIndex).getSuffix() != "") {
									Gui.drawRect(x - 7d, y + 11d, RenderUtils.width()
											- font.getStringWidth(this.getNextEnabledModule(sorted, nextIndex).getName()
											+ " " + this.getNextEnabledModule(sorted, nextIndex).getSuffix())
											- 6d, y + 12d, color);
								} else {
			...
			String text2 = (((Object) EnumChatFormatting.WHITE) + "Speed : "
					+ (Object) ((Object) EnumChatFormatting.GRAY) + String.format("%.2f", lastDist * mc.timer.timerSpeed)
			);
			int ychat;
...
//                	if(!mc.gameSettings.keyBindPlayerList.isKeyDown()) {
			font.drawStringWithShadow(text, 2, RenderUtils.height() - n, new Color(11, 12, 17).getRGB());
			if (!mc.ingameGUI.getChatGUI().getChatOpen()) {
				font.drawStringWithShadow(text2, 2, RenderUtils.height() - n - 12, new Color(11, 12, 17).getRGB());
			}
//                	}
...
//                    font.drawStringWithShadow("[" + direction + "]", font.getStringWidth(String.valueOf(Client.name) + " " + 0.6 + 10), shouldMove ? 15 : 2, new Color(102, 172, 255).getRGB());
			for (int slot = 3, xOffset = 0; slot >= 0; slot--) {
				...
				if (stack != null) {
					mc.getRenderItem().renderItemIntoGUI(stack, RenderUtils.width() / 2 + 15 - xOffset,
							RenderUtils.height() - 55
