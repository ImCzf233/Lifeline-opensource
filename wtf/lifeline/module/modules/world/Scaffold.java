...
import wtf.lifeline.events.world.EventSafeWalk;
...
import wtf.lifeline.utils.InventoryUtils;
...
import wtf.lifeline.utils.sub.Rotation;
...
public class Scaffold extends Module {
    ...
    private void onRotation(EventUpdate event) {
        ...
        if (under == Blocks.air || !under.isFullBlock()) {
            ...
            float yaw = (float) (Math.atan2(z, x) * 180 / Math.PI) - 90 - MathUtil.getRandomFloat(-10, 10) - ((float) (Math.atan2(z, x) * 180 / Math.PI) - 90) % 45;
            float pitch = (float) (Math.atan2(y, dist) * 180 / Math.PI) + MathUtil.getRandomFloat(-10, 10);
            if (yaw < 0) yaw += 360;
